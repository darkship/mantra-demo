...
describe('core.components.register', () => {
  const handler = {
    ...
    handleEmail: spy(),
    handlePassword: spy(),
    handlePasswordVerification: spy(),
  };
  ...
  it('should display the right data and no error', () => {
    ...
    expect(register.find('input[type="email"]').props().value)
      .to.equal(props.email);
    expect(register.find({placeholder: 'Password'}).props().value)
      .to.equal(props.password);
    expect(register.find({placeholder: 'Password verification'}).props().value)
      .to.equal(props.passwordverification);
