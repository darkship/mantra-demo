...
describe('core.actions.login', () => {
  it('login should log in with Meteor',()=>{
  		...
  		const Meteor ={loginWithPassword:function(user,pwd,cb){
        cb && cb('error')
      }};
  		const Store ={dispatch:spy(),getState:stub().returns({LoginReducer:{email:'123',password:'123'}})};
      ...
      expect(Store.dispatch.calledWith({type:'SET_ERROR',error:'error'})).to.be.true;
  ...
  it('setPassword should store email',()=>{
  	...
  	expect(Store.dispatch.callCount).to.equal(1);
	  expect(Store.dispatch.calledWith({type:'SET_PASSWORD',password})).to.be.true;
  })
});
