...
export default {
  ...
  setEmail({Store}, email) {
    if (checkIfEmail(email)) {
      ...
      Store.dispatch({type: 'SET_INVALID_EMAIL', email});
    }
  },
  setPassWord({Store}, password) {
    if (checkPasswordFormat(password)) {
      ...
      Store.dispatch({type: 'SET_INVALID_PASSWORD', password});
    }
  },
  setPassWordVerification({Store}, password) {
    const state = Store.getState().RegisterReducer;
    if (state.password == password) {
      Store.dispatch({type: 'SET_PASSWORD_VERIFICATION',
        passwordverification: password});
    } else {
      Store.dispatch({type: 'SET_PASSWORD_VERIFICATION_ERROR',
        passwordverification: password});
    }
  },
  register({Meteor, Store, Accounts, FlowRouter}) {
    const state = Store.getState().RegisterReducer;
    if (checkIfEmail(state.email) && checkPasswordFormat(state.password)
      && state.password == state.passwordverification) {
